#!/usr/bin/env python3
"""Revert platform planks from <dialog> modal back to inline CSS accordion."""
...
for c in cards:
    ...
    # --- opening: bare icon/h3 + modal trigger + dialog → button wrapper + plank-body div ---
    old_open = (
    ...
    assert old_open in html, f'ABORT: opening not found for dialog {did!r}'
    html = html.replace(old_open, new_open, 1)

    # --- closing: dialog-body close + dialog close → plank-body close ---
    old_close = (
    ...
    assert old_close in html, f'ABORT: closing not found for dialog {did!r}'
    html = html.replace(old_close, new_close, 1)

print('Card HTML reverts: OK')
...
assert old_css in html, 'ABORT: modal CSS block not found'
...
assert old_js in html, 'ABORT: modal JS block not found'
...
print('\nDone. SSR still intact — essays hidden by CSS, not conditional rendering.')
print('grep -ic "my fellow viral particles" index.html  →', end=' ')
import subprocess
result = subprocess.run(['grep', '-ic', 'my fellow viral particles', 'index.html'], capture_output=True, text=True)
